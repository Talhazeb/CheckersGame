...
player::player(int plyr, graphics*tpawn, graphics*tkpawn, graphics*tborder) : player_id(plyr)
{
...
void player::status(player *opp)
{
	...
	erazemove(11); //clear previous move
...
int player::emptyright(pawnmove right, int eatflag) 
{
	for (int i = 0; i < PAWN_NUMBER; ++i)
	{
...
bool player::pawn_rightright(pawnmove array_pawnn, bool first)
{
	...
	if (i == -2)
	{
		if (!eatormove)
		{
			...
			player_pawn[index].path_pawn.push_back(path{beginpath, array_pawnn, holdid, first, multi});
		}
	}
	...
	if (i >= 0)
	{
		...
		if (player_pawn[index].king == true)
		{
			magic = magic * (-1);
